...
- Data Entry can now append manual/imported measurements to the workspace file layer, and Dashboard periodic testing plus `/comparison` now read active workspace measurements through `src/lib/workspace-measurement-store.ts`. Admin, remaining Settings, other Dashboard areas, and cache-clear reopen verification still need to become workspace-backed before the system can be treated as data-persistent end to end.
