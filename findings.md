...
- Data Entry can now append manual/imported measurements to the workspace file layer and restore import history from workspace `importBatches`; Dashboard periodic testing plus `/comparison` now read active workspace measurements through `src/lib/workspace-measurement-store.ts`; the Dashboard top athlete selector reads workspace `athletes`; Dashboard top date/athlete filters now drive Dashboard daily monitoring and Dashboard-owned periodic testing measurement queries; Dashboard daily monitoring now derives current `DailyData` fields from workspace measurements where available; Settings main preferences now read/write the active workspace; Admin definition edits now write workspace metric/action/category definitions that Data Entry consumes through `src/lib/workspace-definition-config.ts`; and Admin athlete/session edits now write workspace `athletes`, `teams`, and `testSessions`.
- Dashboard daily monitoring maps current daily workspace metrics (`hrv_rmssd`, `hr_resting`, `sleep_score`, `rpe`, `readiness_score`, `energy_score`, `soreness_score`, `confidence_score`, `training_load`, `session_duration_min`, `acwr`, and `training_monotony`) into the legacy daily chart shape. More advanced derived training-load formulas should be centralized later under PL-012 rather than expanded inside the dashboard component.
