...
- Data Entry can now append manual/imported measurements to the workspace file layer and restore import history from workspace `importBatches`; Dashboard periodic testing plus `/comparison` now read active workspace measurements through `src/lib/workspace-measurement-store.ts`; the Dashboard top athlete selector reads workspace `athletes`; Dashboard top date/athlete filters now drive Dashboard daily monitoring and Dashboard-owned periodic testing measurement queries; Dashboard daily monitoring now derives HRV, RHR, sleep score, and RPE-backed stress series from workspace measurements where available; Settings main preferences now read/write the active workspace; Admin definition edits now write workspace metric/action/category definitions that Data Entry consumes through `src/lib/workspace-definition-config.ts`; and Admin athlete/session edits now write workspace `athletes`, `teams`, and `testSessions`. Several daily-monitoring fallback fields still need to become workspace-backed before the system can be treated as data-persistent end to end.
- Dashboard daily monitoring currently maps only available daily workspace metrics (`hrv_rmssd`, `hr_resting`, `sleep_score`, and `rpe`) into the legacy daily chart shape. Readiness, energy, soreness, confidence, load, ACWR, and monotony still use fallback/mock fields until those metrics and derived calculations are modeled explicitly.
