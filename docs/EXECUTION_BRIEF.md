...
- `src/lib/measurement-store.ts` 已建立第一版共享 mock `Measurement[]` store 和 selector。Data Entry 手动录入和 Excel 导入确认已能生成 `Measurement[]` 并在 PL-010 中写入本地 JSON workspace；Dashboard periodic testing 和 `/comparison` 已开始读取活动 workspace；Admin 定义库、运动员档案和测试批次已开始写入 workspace；其他 Dashboard 区块还没有迁移为 workspace/selector 消费者。
- `src/lib/workspace-file.ts`、`src/lib/workspace-store.tsx`、`src/lib/workspace-measurement-store.ts` 和 `src/components/WorkspaceFileBar.tsx` 已建立第一版本地 `performancelab.workspace.json` 文件层和到 measurement selector 的读取 adapter。它支持 File System Access API 的创建、打开、保存、另存为，以及不支持浏览器的 JSON 导入/导出降级；Settings 的外观偏好、人体图、显示阈值、通知规则、系统统计和导出入口已接入活动 workspace；Admin 定义库新增/编辑/删除指标已写入 `metricDefinitions`、`testActionCategories`、`testActions`，运动员档案和测试批次已写入 `athletes`、`teams`、`testSessions`，Data Entry 指标选择和导入解析会优先读取活动 workspace 定义。当前仍需补剩余核心页面读取路径、导入历史消费和清缓存后重连恢复验证。
...
如果用户没有指定任务，下一轮建议从第一个状态不是 `Done` 的 P0/P1 任务开始；当前默认继续 `PL-010`。本轮已完成本地 JSON workspace 的基础文件层、全局文件栏、Data Entry 写入、Settings 主要配置读写、Dashboard periodic testing/`/comparison` 对活动 workspace 测量数据的读取、Admin 定义库到 Data Entry 定义消费路径，以及 Admin 运动员档案/测试批次的 workspace 读写，但还不能把 PL-010 判定为 `Done`，因为其他核心页面读取路径、导入历史消费和清缓存后重连恢复验证仍未完成。
...
- 是否已有未提交改动。
- 是否存在近期文档更新改变了优先级。
- Dashboard 其他区块、导入历史和 selector 哪些路径仍只读 mock/local state，而不是 workspace。
