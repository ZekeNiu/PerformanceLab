...
- `src/lib/workspace-file.ts`、`src/lib/workspace-store.tsx`、`src/lib/workspace-measurement-store.ts` 和 `src/components/WorkspaceFileBar.tsx` 已建立第一版本地 `performancelab.workspace.json` 文件层和到 measurement selector 的读取 adapter。它支持 File System Access API 的创建、打开、保存、另存为，以及不支持浏览器的 JSON 导入/导出降级；Settings 的外观偏好、人体图、显示阈值、通知规则、系统统计和导出入口已接入活动 workspace；Admin 定义库新增/编辑/删除指标已写入 `metricDefinitions`、`testActionCategories`、`testActions`，运动员档案和测试批次已写入 `athletes`、`teams`、`testSessions`，Data Entry 指标选择、导入解析和导入历史会优先读取活动 workspace，Dashboard 顶部运动员选择器会优先读取 workspace 运动员。当前仍需补剩余核心页面读取路径和清缓存后重连恢复验证。
