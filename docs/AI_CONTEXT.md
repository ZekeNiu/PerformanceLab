...
- `src/lib/workspace-file.ts`, `src/lib/workspace-store.tsx`, `src/lib/workspace-measurement-store.ts`, `src/lib/workspace-definition-config.ts`, and `src/components/WorkspaceFileBar.tsx` are the first local-data-file layer. The intended first-stage authority is a user-visible JSON file, not IndexedDB/localStorage; Dashboard periodic testing and `/comparison` now read active workspace measurements, the Dashboard top athlete selector reads workspace athletes, Dashboard top date/athlete filters drive Dashboard daily monitoring and Dashboard periodic testing measurement queries, Dashboard daily monitoring reads workspace HRV/RHR/sleep/RPE daily measurements where available, Settings main preferences now read/write the active workspace, Admin definition edits write workspace metric/action/category definitions consumed by Data Entry, and Admin athlete/session edits write workspace athletes/teams/test sessions. The JSON import fallback has been verified in fresh browser contexts after simulated cache clearing. Several daily-monitoring derived/fallback fields are still open.
