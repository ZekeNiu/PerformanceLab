...
- Data Entry now uses workspace-backed test action categories and metrics for manual indicator selection and import resolution. Manual saves and Excel confirmation generate domain-model `Measurement[]` through `src/lib/data-entry-measurements.ts` and, as of PL-010, append to the local JSON workspace layer when the user creates or opens `performancelab.workspace.json`; Data Entry import history now reads workspace `importBatches`.
- `src/lib/workspace-file.ts`, `src/lib/workspace-store.tsx`, `src/lib/workspace-measurement-store.ts`, `src/lib/workspace-definition-config.ts`, and `src/components/WorkspaceFileBar.tsx` are the first local-data-file layer. The intended first-stage authority is a user-visible JSON file, not IndexedDB/localStorage; Dashboard periodic testing and `/comparison` now read active workspace measurements, the Dashboard top athlete selector reads workspace athletes, Dashboard top date/athlete filters drive Dashboard daily monitoring and Dashboard periodic testing measurement queries, Dashboard daily monitoring reads current daily monitoring fields from workspace measurements where available, Settings main preferences now read/write the active workspace, Admin definition edits write workspace metric/action/category definitions consumed by Data Entry, and Admin athlete/session edits write workspace athletes/teams/test sessions. The JSON import fallback has been verified in fresh browser contexts after simulated cache clearing. PL-010 is complete.
- `src/lib/availability-matrix.ts` is the first test-content availability boundary. Initial workspace sessions are assigned default test batteries, periodic mock measurements carry `testActionId`/`batteryId` where the default action config can resolve them, and Dashboard periodic cross-sectional comparison now defaults to common available metrics while showing an availability matrix with common available, missing, partial, and incompatible rows. PL-011 is complete.
- `src/lib/derived-metric-formulas.ts` now provides the first derived metric formula execution boundary. Workspace measurement stores and the availability matrix can generate computed `Measurement[]` rows from raw workspace measurements using registered definitions, including dependency specs, optional dimension filters, missing-data policy, quality flags, notes, and computation reports. Initial workspaces seed squat relative strength, bench relative strength, and BMI derived definitions; `/comparison` can display derived relative strength metrics through the existing measurement selector and mark them commonly available. PL-012 is complete.
- PL-013 is complete: Dashboard top-level date, athlete, and test-session filters now feed the shared measurement query; the test-session picker reads workspace `testSessions` and passes `sessionIds`; periodic overview display config changes real `radar-chart`/`bar-chart`/`table` output; periodic category cards switch real metric ids; low-dimensional radar outputs downgrade to bar/table; non-functional Dashboard config menus were removed. The next architecture task is PL-014 daily monitoring UI restructuring.
